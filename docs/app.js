 ...
 * 持股只存 localStorage；無 .env / Token / Chat ID / 私人持股。K 線以 Canvas 即時繪製（不產生圖片檔）。
 */
...
function buildIndicators(bars) {
  ...
  return {
    ...
    ma5, ma10, ma20, ma60, rsi: r, support, resistance, volRatio,
    distMa20: ma20 ? (last - ma20) / ma20 * 100 : null,
  };
}
...
function decide(ind, fund, market) {
  ...
  return { position, score, risk, action, operation, trendUp, overheated, hot, belowMa20, nearSupport, nearResistance, farFromMa20 };
}

/* ---------- 基本面 ---------- */
function fundamentalExplain(fund, market) {
  if (market !== "TW") return "美股基本面（EPS / P/E / P/B / 殖利率 / 營收）純前端版資料不足，完整基本面建議搭配完整版查詢。";
  const p = [];
  if (fund.pe != null) p.push(`P/E ${fmt(fund.pe, 1)} ${fund.pe < 12 ? "屬合理偏低" : fund.pe <= 20 ? "屬合理" : fund.pe <= 30 ? "略偏高" : "偏高"}`);
  ...
  if (fund.pe != null && fund.pb != null) {
    if (fund.pe <= 20 && fund.pb <= 1.5) s += "整體估值偏低、屬相對便宜位置，但仍需搭配股價位置判斷。";
    else if (fund.pe > 30 || fund.pb > 3) s += "整體估值偏高，需留意回檔風險。";
    else s += "代表目前不是明顯低估位置，須搭配股價位置與技術面判斷進場時機。";
  }
  return s;
}
...
function technicalExplain(ind) {
  const { close, ma5, ma10, ma20, rsi: r } = ind;
  const p = [];
  if (ma20 && close > ma20) p.push("目前股價站上 MA20"); else if (ma20) p.push("目前股價跌破 MA20");
  if (ma5 && ma10 && ma20) { if (ma5 > ma10 && ma10 > ma20) p.push("且 MA5 > MA10 > MA20，短線維持多頭排列"); else if (ma5 < ma10 && ma10 < ma20) p.push("且均線呈空頭排列"); }
  let s = p.join("，") + "；";
  if (r != null) { if (r >= 75) s += `但 RSI 已達 ${fmt(r, 0)}，代表短線過熱，不適合直接追高，較適合等待拉回`; else if (r <= 30) s += `RSI ${fmt(r, 0)} 偏低、短線超賣，可留意反彈`; else s += `RSI ${fmt(r, 0)} 位於健康區間`; }
  return s + "。";
}

/* ---------- 籌碼面 ---------- */
function buildChip(inst, margin) {
  // inst: 最新一日三大法人；margin: 最新一日融資融券
  if (!inst && !margin) {
    return {
      ok: false,
      note: "籌碼面資料不足，純前端版本目前無法完整取得三大法人與融資融券資料。",
  ...
  // 解讀
  const buys = (inst && inst.foreign > 0) || (inst && inst.trust > 0);
  const marginUp = margin && margin.marginChg > margin.marginBal * 0.03;
...
/* ---------- 可買 / 不建議買 ---------- */
function buyReasons(ind, fund, d, market) {
  const r = [];
...
function avoidReasons(ind, fund, d, market) {
  const r = [];
...
function priceZones(ind) {
  ...
  else if (close >= res * 0.97) posNote = "接近短線高檔 / 壓力，等待回落較安全";
  else posNote = "位於合理觀察區間，建議分批、勿追高";
  return {
    support: sup, midLow: sup, midHigh: res, resistance: res, chaseAbove: res * 1.0,
...
function conclusion(ind, d) {
  ...
  let act;
  if (d.overheated) act = "若已持有可續抱觀察；若尚未進場，不建議直接追高，較適合等待回落接近支撐區再分批，新進場需保守";
  else if (d.belowMa20) act = "建議觀望、待站回均線並確認支撐後再評估";
  else if (d.nearSupport) act = "可分批布局並嚴設停損；跌破支撐則先離場";
  else act = "可分批觀察、避免追高，並留意均線與量能變化";
  return `目前屬於「${state}」。${act}。`;
}

/* ---------- K 線（Canvas，不產生圖片檔） ---------- */
function drawKline(canvas, bars) {
  if (!canvas || !bars || bars.length < 2) return;
  canvas._bars = bars;   // 供 resize 重繪
  ...
  const ma5 = ma5all.slice(offset), ma20 = ma20all.slice(offset);
  const lows = data.map((b) => b.l), highs = data.map((b) => b.h), vols = data.map((b) => b.v);
  const support = Math.min(...bars.slice(-20).map((b) => b.l)), resistance = Math.max(...bars.slice(-20).map((b) => b.h));
  ...
  const padL = 8, padR = 52, padT = 8;
  const priceH = cssH * 0.72, volTop = priceH + 14, volH = cssH - volTop - 16;
  const W = cssW - padL - padR;
  const n = data.length, slot = W / n, bw = Math.max(2, slot * 0.62);

  let pMin = Math.min(...lows, support), pMax = Math.max(...highs, resistance);
  const pad = (pMax - pMin) * 0.06 || 1; pMin -= pad; pMax += pad;
  const yP = (v) => padT + (pMax - v) / (pMax - pMin) * (priceH - padT);
  const vMax = Math.max(...vols) || 1;
  const yV = (v) => volTop + (1 - v / vMax) * volH;

  // 格線 + 價格刻度
  ctx.strokeStyle = "#2a2f3a"; ctx.fillStyle = "#97a0b0"; ctx.font = "10px system-ui"; ctx.lineWidth = 1;
  for (let i = 0; i <= 4; i++) {
    const v = pMax - (pMax - pMin) * i / 4, y = yP(v);
    ctx.beginPath(); ctx.moveTo(padL, y); ctx.lineTo(padL + W, y); ctx.stroke();
    ctx.fillText(v.toFixed(1), padL + W + 4, y + 3);
  }
  // 支撐 / 壓力虛線
  ctx.setLineDash([4, 3]);
  ctx.strokeStyle = "#36c275"; ctx.beginPath(); ctx.moveTo(padL, yP(support)); ctx.lineTo(padL + W, yP(support)); ctx.stroke();
  ctx.fillStyle = "#36c275"; ctx.fillText("支撐 " + support.toFixed(1), padL + 2, yP(support) - 3);
  ctx.strokeStyle = "#ff7a45"; ctx.beginPath(); ctx.moveTo(padL, yP(resistance)); ctx.lineTo(padL + W, yP(resistance)); ctx.stroke();
  ctx.fillStyle = "#ff7a45"; ctx.fillText("壓力 " + resistance.toFixed(1), padL + 2, yP(resistance) + 10);
  ctx.setLineDash([]);
  ...
  const line = (arr, color) => {
    ...
    arr.forEach((v, i) => { if (v == null) return; const x = padL + slot * i + slot / 2, y = yP(v); if (!started) { ctx.moveTo(x, y); started = true; } else ctx.lineTo(x, y); });
    ctx.stroke();
  };
...
async function analyzeTW(code) {
  ...
  // EPS：近 4 季合計 (TTM)
  if (finRaw && finRaw.length) {
    const eps = finRaw.filter((r) => r.type === "EPS").sort((a, b) => a.date < b.date ? -1 : 1);
    if (eps.length) { const last4 = eps.slice(-4).map((r) => num(r.value)).filter((x) => x != null); if (last4.length) fund.eps = last4.reduce((a, b) => a + b, 0); }
  }
  // 營收 YoY
  if (revRaw && revRaw.length >= 13) {
    const sorted = revRaw.slice().sort((a, b) => a.date < b.date ? -1 : 1);
    const lastR = sorted[sorted.length - 1];
    const yoyMatch = sorted.find((r) => r.revenue_month === lastR.revenue_month && r.revenue_year === lastR.revenue_year - 1);
    if (yoyMatch && yoyMatch.revenue) fund.revYoy = (lastR.revenue - yoyMatch.revenue) / yoyMatch.revenue * 100;
  }

  // 三大法人（最新日，股 → 張）
  let inst = null;
  if (instRaw && instRaw.length) {
    const lastDate = instRaw.reduce((m, r) => r.date > m ? r.date : m, "");
    const day = instRaw.filter((r) => r.date === lastDate);
    const net = (names) => day.filter((r) => names.includes(r.name)).reduce((a, r) => a + (r.buy - r.sell), 0) / 1000;
    inst = {
      date: lastDate,
      foreign: net(["Foreign_Investor", "Foreign_Dealer_Self"]),
      trust: net(["Investment_Trust"]),
      dealer: net(["Dealer_self", "Dealer_Hedging"]),
    };
  }
  // 融資融券（最新日）
  let margin = null;
  if (marginRaw && marginRaw.length) {
    const m = marginRaw[marginRaw.length - 1];
    margin = {
      marginBal: num(m.MarginPurchaseTodayBalance), marginChg: num(m.MarginPurchaseTodayBalance) - num(m.MarginPurchaseYesterdayBalance),
      shortBal: num(m.ShortSaleTodayBalance), shortChg: num(m.ShortSaleTodayBalance) - num(m.ShortSaleYesterdayBalance),
    };
  }

  return {
    market: "TW", symbol: code, name: info && info.length ? info[0].stock_name : code,
    industry: info && info.length ? info[0].industry_category : null,
    bars, ind, fund, chip: buildChip(inst, margin),
  };
}
...
async function analyzeUS(sym) {
  ...
  const ind = buildIndicators(bars);
  return {
    market: "US", symbol: sym.toUpperCase(), name: sym.toUpperCase(), industry: null,
    bars, ind, fund: { pe: null, pb: null, dy: null, eps: null, revYoy: null },
    chip: { ok: false, note: "美股無台股式三大法人 / 融資融券制度，純前端版籌碼面資料不足。", explain: "籌碼面主要用來觀察主力資金流向；美股可參考機構持股與放空比，純前端版暫不提供，建議搭配完整版查詢。" },
  };
}
...
async function analyze(symbol, market, cost, qty) {
  ...
  try {
    ...
    drawKline(card.querySelector("canvas"), a.bars);
  } catch (e) {
    card.innerHTML = `<div class="err">⚠️ ${esc(e.message || "分析失敗")}</div>
      <p class="muted">若為流量限制，請稍後再試。</p>`;
  }
}
...
function renderResult(a) {
  ...
  const header = `<div class="rhead">
    ...
    <div class="sumbadges">
      <span class="badge ${actCls}">${esc(d.action)}</span>
      <span>信心 <b>${d.score}</b>/100</span>
      <span>風險 <b>${esc(d.risk)}</b></span>
      <span>位置 <b>${esc(d.position)}</b></span>
    </div>`;

  // 持股
  let hold = "";
  if (a.holding) { const h = a.holding, cls = h.ret >= 0 ? "pnl-up" : "pnl-down";
    hold = `<div class="holding"><b>💼 我的持股</b>　成本 ${fmt(h.cost)}　股數 ${fmt(h.qty, 0)}　市值 ${fmt(h.marketValue)}　<span class="${cls}">報酬 ${fmt(h.ret)}%／損益 ${fmt(h.pnl)}</span>　建議：<b>${esc(h.suggestion)}</b></div>`; }
  ...
  const fundList = m === "TW" ? [
    ...
    `本益比 P/E：${fmt(f.pe, 1)}`, `股價淨值比 P/B：${fmt(f.pb, 2)}`,
    `殖利率：${f.dy != null ? fmt(f.dy, 2) + "%" : "資料不足"}`,
    `營收成長率（YoY）：${f.revYoy != null ? (f.revYoy >= 0 ? "+" : "") + fmt(f.revYoy, 1) + "%" : "資料不足"}`,
  ] : [
  ...
  const techList = [
    `MA5：${fmt(i.ma5)}　MA10：${fmt(i.ma10)}`, `MA20：${fmt(i.ma20)}　MA60：${fmt(i.ma60)}`,
  ...
  let chipBody;
  if (a.chip.ok) {
    const c = a.chip, items = [];
    if (c.foreign != null) items.push(`三大法人（張）：外資 ${signed(c.foreign)}、投信 ${signed(c.trust)}、自營商 ${signed(c.dealer)}`);
    if (c.marginBal != null) items.push(`融資餘額：${thou(c.marginBal)} 張（${signed(c.marginChg)}）`);
    if (c.shortBal != null) items.push(`融券餘額：${thou(c.shortBal)} 張（${signed(c.shortChg)}）`);
    chipBody = ul(items) + `<p class="exp">${esc(c.explain)}</p>`;
  } else {
    chipBody = `<p>${esc(a.chip.note)}</p><p class="exp">${esc(a.chip.explain)}</p>`;
  }
  const chip = `<div class="block"><h4>④ 籌碼面　<small>看主力動向</small></h4>${chipBody}</div>`;

  // ⑤ K 線
  const kline = `<div class="block"><h4>⑤ K 線圖　<small>近 60 日</small></h4>
      <canvas class="kline"></canvas>
      <div class="legend"><span class="lg up">紅 漲</span><span class="lg dn">綠 跌</span>
        <span class="lg ma5">MA5</span><span class="lg ma20">MA20</span>
        <span class="lg sup">支撐</span><span class="lg res">壓力</span></div></div>`;

  // ⑥⑦ 理由
  const buy = `<div class="block buy"><h4>⑥ 為什麼可以買 / 偏多理由</h4>${ul(buyReasons(i, f, d, m))}</div>`;
  const nobuy = `<div class="block nobuy"><h4>⑦ 為什麼不建議買 / 風險理由</h4>${ul(avoidReasons(i, f, d, m))}</div>`;

  // ⑧ 價格區間
  const z = priceZones(i);
  const zones = `<div class="block"><h4>⑧ 價格區間 / 進場參考</h4>${z ? `<pre class="zones">${esc(z.text)}</pre>` : "<p>資料不足</p>"}</div>`;

  // ⑨ 結論
  const concl = `<div class="block conclusion"><h4>⑨ 結論</h4><p>${esc(conclusion(i, d))}</p></div>`;

  return header + hold + decision +
    `<div class="aspects">${fundamental}${technical}${chip}</div>` +
    kline +
    `<div class="aspects two">${buy}${nobuy}</div>` +
    zones + concl +
...
let rzT; window.addEventListener("resize", () => { clearTimeout(rzT); rzT = setTimeout(() => {
  document.querySelectorAll(".kline").forEach((cv) => { if (cv._bars) drawKline(cv, cv._bars); });
}, 150); });
...
$("save").addEventListener("click", () => {
  ...
  const m = $("market").value, cost = parseFloat($("cost").value) || null, qty = parseFloat($("qty").value) || null;
...
$("holdings").addEventListener("click", (e) => {
  ...
  if (a) { e.preventDefault(); const m = a.getAttribute("data-m"), s = a.getAttribute("data-s"); const h = getHoldings().find((x) => x.market === m && x.symbol === s) || {}; analyze(s, m, h.cost, h.qty); }
});
