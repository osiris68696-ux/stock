...
與 Telegram 私人版**分流**：Web 版讓使用者自行輸入股票代號 / 自訂持股，**不顯示我的私人持股、不碰 Telegram / .env**。

```powershell
python web_app.py                 # 本機 http://127.0.0.1:8000
python web_app.py 0.0.0.0 8000    # 對外 (請自行評估部署安全)
```
...
| 版本 | 用途 | 持股 | 推送 |
|---|---|---|---|
| **Telegram 私人版** | 自己看、每日自動 | 我的持股 (0050/0056/00878/2887) | Telegram |
| **Web 公開版** | 一般網友、自行查詢 | 使用者自訂 (localStorage) | 無 (網頁顯示) |

> GitHub 僅作原始碼管理 / 備份 / 部署來源，**所有 git 操作由你手動執行**（本系統不會自動 push / 建 repo / 改 GitHub）。
